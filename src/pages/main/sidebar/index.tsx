...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar }) => {
  return (
    <SidebarContainer isOpen={isOpen}>
      ...
      />
      <Flex direction="column" alignItems="center" justifyContent="flex-start" mt={8} width="100%">  {/* 가로로 가운데 정렬 */}
        <Box textAlign="center" mb={4}>
          <p style={{ fontSize: '24px', fontWeight: 'bold' }}>경북대 컴퓨터학부</p>  {/* 폰트 크기와 굵기 조정 */}
          <p style={{ fontSize: '24px', fontWeight: 'bold' }}>학사 정보 AI</p>  {/* 폰트 크기와 굵기 조정 */}
        </Box>
        <img src={knuLogo} alt="KNU Logo" style={{ width: '100px', height: '100px' }} />
        <Divider />  {/* 구분선 추가 */}
      </Flex>
    </SidebarContainer>
  );
...
const SidebarContainer = styled(Box)<{ isOpen: boolean }>`
  ...
  position: fixed;
  top: 0;
  left: ${({ isOpen }) => (isOpen ? '0' : '-340px')};  // 열림/닫힘 상태에 따라 사이드바 위치 변경
